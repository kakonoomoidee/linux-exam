...
async function migrate() {
  ...
  //     Drops users_kelas_code_chk first — the interim loose check from the
  //     since-reverted custom-class-code feature.
  //  2. questions unique key gains `ucp` so UCP 1 and UCP 2 can both number
  //     their questions 1..N within the same variant.
  await sequelize.query(`
    DO $$ BEGIN
      IF EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'users_kelas_code_chk') THEN
        ALTER TABLE users DROP CONSTRAINT users_kelas_code_chk;
      END IF;
